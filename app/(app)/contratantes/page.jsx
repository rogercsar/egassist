...
export default async function ContratantesPage({ searchParams }) {
  ...
  async function createContratante(formData) {
    'use server'
    ...
    if (!uid || !nome) {
      redirect('/contratantes?error=' + encodeURIComponent('Preencha o nome.'))
      return
    }
    ...
    if (error) {
      redirect('/contratantes?error=' + encodeURIComponent(error.message || 'Falha ao criar contratante.'))
      return
    }

    redirect('/contratantes?saved=1')
  }

  const items = contratantes.map(c => ({ label: c.nome, value: '' }))
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-semibold text-brand-black">Contratantes</h1>

      {saved && (
        <FlashBanner message="Contratante criado com sucesso." variant="success" autoHideMs={4000} />
      )}
      {errorMsg && (
      ...
      <Card title="Lista">
        ...
        <BrandList items={items} emptyText="Nenhum contratante encontrado." />
      ...
      <Card title="Criar Contratante">
        <ResetOnSaved saved={saved} formIds={["form-contratante"]} />
        <form id="form-contratante" action={createContratante} className="flex flex-col gap-2 max-w-md">
